 ...
 *
 * Uses the drive.file scope (see BackupRestoreViewModel.DRIVE_SCOPE), which
 * means this app can only ever see/manage files IT created — never the rest
 * of the signed-in user's Drive. All backups are named with BACKUP_PREFIX so
 * they're easy to find, sort by recency, and clean up.
 */
object DriveBackupHelper {
    private const val BACKUP_PREFIX = "Triangle_Backup_"
